    ...
    *,
    ...
    context = {
        ...
        "a_cappella_arrangement_rules": [
            ...
            "Plan every vocal target as an a cappella track, not as a generic harmony filler. The target must have a clear role in the full ensemble texture.",
            "Use other registered tracks as mandatory context for rhythm, phrase placement, vertical harmony, and articulation.",
            ...
            "Preserve short repeated-note articulation when it is part of the context groove or lyric rhythm; sustain plans still need audible attacks on structural beats.",
            ...
            "Candidates must be meaningfully different by role and texture: context-locked rhythm, stable blend, independent counterline, upper blend, pad/sustain, rhythmic echo, or hook support.",
            ...
            "candidate_directions": [
                {
                    ...
                    "texture": "block_harmony|counterline|pad_sustain|rhythmic_echo|hook_support",
                    "rhythm_role": "context_lock|stable_pulse|independent_motion|sustain_with_attacks|hook_or_riff",
    ...
    messages = [
        {
            ...
            "content": (
                ...
                "Use phrase, texture, rhythm-role, and measure-level harmonic intent with distinct candidate goals. "
                ...
                "Do not generate pitch events, beats, MIDI, prose paragraphs, or markdown."
            ),
        },
        {
            "role": "user",
...
def _build_plan_revision_payload(
    *,
    ...
    context = {
        ...
        "candidate_count": candidate_count,
        "a_cappella_arrangement_rules": [
            "Revise as an a cappella arranger: singability, independent lines, ensemble texture, rhythm articulation, and candidate diversity matter.",
            "Keep the plan bounded; final pitch events are generated by deterministic code.",
        ],
        "context_tracks": [
            _summarize_track(slot_id, events)
            for slot_id, events in sorted(context_events_by_slot.items())
        ...
        "revision_checklist": [
            ...
            "Check that the candidate directions are meaningfully different by role, rhythm, and texture.",
...
def _summarize_track(slot_id: int, events: list[TrackPitchEvent]) -> dict[str, Any]:
    pitched_events = [event for event in events if event.pitch_midi is not None and not event.is_rest]
    pitches = [event.pitch_midi for event in pitched_events if event.pitch_midi is not None]
    return {
        "slot_id": slot_id,
        ...
        "events": [
            {
                ...
                "absolute_beat": event.beat,
                "duration_beats": event.duration_beats,
                "label": event.spelled_label or event.label,
                "pitch_midi": event.pitch_midi,
            }
            for event in pitched_events[:64]
        ],
    }
...
def _summarize_measures(
    ...
    *,
    ...
    for slot_id, events in context_events_by_slot.items():
        for event in events:
            if event.pitch_midi is None or event.is_rest:
                continue
            measure_index = event.measure_index
            if measure_index is None:
                measure_index = int((max(event.beat, 1) - 1) // beats_per_measure) + 1
            ...
            summary["pitch_classes"].add(event.pitch_midi % 12)

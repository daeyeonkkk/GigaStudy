...
class EvidenceRoundPaths:
    root: Path
    readme: Path
    real_evidence_plan_path: Path
    real_evidence_checklist_path: Path
...
def resolve_evidence_round_paths(round_root: Path) -> EvidenceRoundPaths:
    ...
    return EvidenceRoundPaths(
        ...
        real_evidence_plan_path=resolved_root / "REAL_EVIDENCE_PLAN.md",
        real_evidence_checklist_path=resolved_root / "REAL_EVIDENCE_CHECKLIST.md",
    ...
    *,
    ...
    return "\n".join(
        [
            ...
            "## Batch Helpers",
            "",
            "- `REAL_EVIDENCE_PLAN.md` gives the one-shot collection order for future real-user data sessions.",
            "- `REAL_EVIDENCE_CHECKLIST.md` is the short tick-off list for the same batch.",
            "",
            "## Human Rating",
            "",
...
def render_real_evidence_plan(*, round_id: str, repo_root: Path) -> str:
    api_root = repo_root / "apps" / "api"

    return "\n".join(
        [
            f"# Real Evidence Batch Plan: {round_id}",
            "",
            "Use this plan when real singer data and real-device validation become available again.",
            "The goal is to collect the open Phase 9 and Phase 10 evidence in one coordinated batch instead of reopening the workflow piece by piece.",
            "",
            "## What This Batch Should Close",
            "",
            "- real human vocal fixtures or a trusted human-rating corpus",
            "- threshold calibration review against real human raters",
            "- native Safari / WebKit validation on real Apple hardware",
            "- broader real microphone and output-route variability evidence",
            "",
            "## Before Collection Day",
            "",
            "1. Pick one round id and keep every artifact under this round only.",
            "2. Choose 2-4 real guide/take pairs that represent meaningful cases:",
            "   - one mostly stable take",
            "   - one sharp or overshooting take",
            "   - one flat or recovery take",
            "   - optionally one expressive / vibrato-heavy take",
            "3. Decide who will do note-level rating and who will run browser/hardware checks.",
            "4. Confirm the alpha environment is still usable for recording, playback, and ops validation logging.",
            "",
            "## Collection Order",
            "",
            "### 1. Seed The Round From Real Studio Data",
            "",
            "Export the chosen guide/take pairs from the product first, so the round starts from canonical WAVs and generated review material instead of hand-copied files.",
            "",
            "```bash",
            f"cd {api_root}",
            "uv run python scripts/export_project_case_to_evidence_round.py --round-root <round> --project-id <project-id> --take-track-id <take-track-id>",
            "```",
            "",
            "Repeat that export for every real take that should enter the batch.",
            "",
            "### 2. Hand Off Human Rating Packets",
            "",
            "For each exported case:",
            "",
            "- send the generated review packet HTML",
            "- send the seeded rating sheet CSV",
            "- keep note references neutral; do not send system verdict text as instruction",
            "- collect at least 2-3 raters per case when possible",
            "",
            "### 3. Run Native Browser And Hardware Validation",
            "",
            "Use the same round's `environment-validation/environment_validation_runs.csv` as the single intake sheet.",
            "",
            "Minimum target matrix for the first real batch:",
            "",
            "- Windows + Chrome + USB microphone + wired headphones",
            "- Windows + Firefox + built-in microphone + built-in speakers",
            "- macOS + Safari + built-in microphone + built-in speakers",
            "- macOS + Safari + Bluetooth output",
            "",
            "If one cell is skipped, record that honestly in the sheet instead of silently dropping it.",
            "",
            "### 4. Refresh The Round After Collection",
            "",
            "Once human-rating CSV rows and hardware-validation CSV rows are filled in, regenerate the whole support bundle in one pass.",
            "",
            "```bash",
            f"cd {api_root}",
            "uv run python scripts/refresh_evidence_round.py --round-root <round>",
            ...
            "```",
            "",
            "### 6. Build Browser / Hardware Evidence",
            "",
            "Prefer ops UI import for the final storage path, but keep the round-local preview first.",
            "",
            "```bash",
            f"cd {api_root}",
            "uv run python scripts/refresh_evidence_round.py --round-root <round>",
            ...
            "```",
            "",
            "Then export from ops:",
            "",
            "- environment diagnostics report",
            "- environment validation packet",
            "- browser environment claim gate",
            "- compatibility release-note draft",
            "",
            "## Review Gate",
            "",
            "Do not close checklist items just because the round exists.",
            "Only begin closure review when all of the following are true:",
            "",
            "- real guide/take audio is present and verified",
            "- human-rating corpus is built from real raters",
            "- threshold-fit report exists for that corpus",
            "- human-rating claim gate is reviewed",
            "- native Safari / WebKit evidence exists",
            "- real hardware microphone evidence exists",
            "- browser / hardware claim gate is reviewed",
            "",
...
def render_real_evidence_checklist() -> str:
    return "\n".join(
        [
            "# Real Evidence Batch Checklist",
            "",
            "## Round Setup",
            "",
            "- [ ] Round folder created",
            "- [ ] Alpha environment chosen for the batch",
            "- [ ] Target real projects / takes selected",
            "",
            "## Human Rating Collection",
            "",
            "- [ ] Real guide/take pairs exported into the round",
            "- [ ] Review packet HTML checked for each case",
            "- [ ] Rating sheet shared with raters",
            "- [ ] At least one case has 2+ raters",
            "- [ ] At least one case has 3 raters",
            "- [ ] Generated corpus built from the collected sheet",
            "- [ ] Corpus inventory reviewed for missing audio or weak coverage",
            "- [ ] Calibration summary generated",
            "- [ ] Threshold-fit report generated",
            "- [ ] Human-rating claim gate reviewed",
            "- [ ] Human-rating evidence bundle generated",
            "",
            "## Browser / Hardware Collection",
            "",
            "- [ ] Windows + Chrome + USB mic + wired headphones run recorded",
            "- [ ] Windows + Firefox + built-in path run recorded",
            "- [ ] macOS + Safari + built-in path run recorded",
            "- [ ] macOS + Safari + Bluetooth output run recorded",
            "- [ ] Environment validation CSV reviewed",
            "- [ ] Round-local environment packet preview generated",
            "- [ ] Round-local browser / hardware claim-gate preview generated",
            "- [ ] Validation rows imported into ops",
            "- [ ] Ops environment validation packet exported",
            "- [ ] Ops browser compatibility release-note draft exported",
            "- [ ] Ops browser / hardware claim gate reviewed",
            "",
            "## Closeout",
            "",
            "- [ ] Remaining missing evidence is written down honestly",
            "- [ ] Checklist items to close are named explicitly",
            "- [ ] Foundation summary update is prepared from evidence, not guesswork",
        ]
    )
...
def create_evidence_round_scaffold(
    *,
    ...
    scaffold_paths.real_evidence_plan_path.write_text(
        render_real_evidence_plan(round_id=normalized_round_id, repo_root=resolved_project_root),
        encoding="utf-8",
    )
    scaffold_paths.real_evidence_checklist_path.write_text(
        render_real_evidence_checklist(),
        encoding="utf-8",
    )

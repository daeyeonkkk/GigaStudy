...
def estimate_alignment_offset(answer: list[TrackPitchEvent], performance: list[TrackPitchEvent]) -> float:
...
def _score_voice_leading(
    ...
    *,
    target_slot_id: int,
    references: dict[int, list[TrackPitchEvent]] | None = None,
...
def _is_structural_parallel_motion(
    ...
    current: TrackPitchEvent,
    previous_reference: TrackPitchEvent,
    current_reference: TrackPitchEvent,
) -> bool:
    return min(
...
def _has_chord_coverage_issue(
    ...
    *,
    ...
    if len(evaluation.context_events) < 2:
        return False
    if not _is_structural_harmony_position(
        evaluation,
        time_signature_numerator=time_signature_numerator,
    ...
    if reference_count == 0:
        issues.append(
            ReportIssue(
                ...
                issue_type="harmony",
                severity="error",
                message="Harmony scoring needs at least one registered reference track.",
            )
        )
    if not performance:
        issues.append(
            ReportIssue(
                ...
                issue_type="missing",
                severity="error",
                message="No performance events were detected for harmony scoring.",
            )
        )
        return issues
        ...
        if leap > 12 or (leap > 9 and not (low <= current.pitch_midi <= high)):
            issues.append(
                ReportIssue(
                    ...
                    severity="warn",
                    performance_source_event_id=current.id,
                    ...
                    message="The melodic leap is large and should be reviewed for singability.",
                )
            )
    issues.extend(
    ...
    for previous, current in zip(performance, performance[1:], strict=False):
        ...
        for reference_events in references.values():
            ...
            if previous_interval in {0, 7} and current_interval == previous_interval:
                ...
                interval_name = "octave" if current_interval == 0 else "fifth"
                issues.append(
                    ReportIssue(
                        at_seconds=actual_at,
                        ...
                        message=f"Parallel {interval_name} motion was detected against a reference voice.",
                    )
                )
                break
    return issues
    ...
    for match in matches:
        ...
        issues.append(
            ReportIssue(
                ...
                severity="warn",
                answer_source_event_id=match.answer.id,
                performance_source_event_id=match.performance.id,

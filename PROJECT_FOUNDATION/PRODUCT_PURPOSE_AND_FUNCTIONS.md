...
사용자는 BPM/meter를 입력해 빈 studio로 시작하거나, 지원 source를
업로드해 engine이 studio를 seed하게 한다. `업로드 후 시작`은 source file이
있을 때만 노출/활성화되어야 한다.

### 2. Material 등록

녹음, document import, MIDI, MusicXML, PDF, audio upload, AI generation은

...
def render_document_source_preview(
    source_path: Path,
    *,
    page_index: int = 0,
    ...
    if fitz is None:
        raise DocumentPreviewError("Document preview rendering is unavailable.")
    if not source_path.exists() or not source_path.is_file():
        raise DocumentPreviewError("Document source file is missing.")
    if page_index < 0:
        raise DocumentPreviewError("Document preview page index is invalid.")

    try:
        document = _open_preview_document(source_path)
    except Exception as error:  # noqa: BLE001 - PyMuPDF raises several concrete errors.
        raise DocumentPreviewError("Document source file cannot be opened for preview.") from error

    with document:
        if document.page_count <= 0:
            raise DocumentPreviewError("Document source file has no previewable pages.")
        if page_index >= document.page_count:
            raise DocumentPreviewError("Document preview page is out of range.")

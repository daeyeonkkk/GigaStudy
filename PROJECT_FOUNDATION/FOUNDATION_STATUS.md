...
- The alpha deployment track now has repo-side container implementation plus direct object-storage uploads for S3-compatible storage backends, but it still lacks Docker-backed container verification and one verified HTTPS staging environment on the chosen stack.
...
7. Use `OPERATIONS/ALPHA_DEPLOYMENT_TARGET.md` as the current source of truth for the low-cost alpha stack, then close the remaining deployment blockers in order: backend container verification and one verified HTTPS staging environment.

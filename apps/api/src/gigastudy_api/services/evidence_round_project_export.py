    ...
    *,
    ...
    return "\n".join(
        [
            ...
            "<head>",
            '  <meta charset="utf-8" />',
            f"  <title>{html.escape(case_id)} 사람 평가 패킷</title>",
            "  <style>",
            "    body { font-family: 'Malgun Gothic', 'Apple SD Gothic Neo', Arial, sans-serif; margin: 32px; color: #111827; }",
            ...
            "    .legend { margin: 20px 0; padding: 16px; border-radius: 12px; background: #fff7ed; border: 1px solid #fed7aa; }",
            "    .legend ul { margin: 8px 0 0; padding-left: 20px; }",
            "  </style>",
            "</head>",
            "<body>",
            f"  <h1>사람 평가 리뷰 패킷: {html.escape(case_id)}</h1>",
            f"  <p><strong>프로젝트:</strong> {html.escape(project.title)}</p>",
            "  <p class=\"hint\">이 패킷은 라운드의 평가 시트와 함께 사용합니다. note index, 목표 음, 오디오 참고만 보여주고 시스템의 sharp/flat 판정 문구는 의도적으로 숨깁니다.</p>",
            "  <div class=\"pair\">",
            "    <div class=\"card\">",
            "      <h2>전체 가이드</h2>",
            ...
            "  </div>",
            "  <div class=\"legend\">",
            "    <strong>권장 입력값</strong>",
            "    <ul>",
            "      <li>시작음 방향: 높음 / 정확 / 낮음 / 판단 어려움</li>",
            "      <li>지속음 방향: 높음 / 정확 / 낮음 / 판단 어려움</li>",
            "      <li>허용도: 양호 / 검토 / 교정 필요 / 판단 어려움</li>",
            "    </ul>",
            "  </div>",
            "  <h2>노트별 리뷰</h2>",
            "  <p class=\"hint\">먼저 전체 테이크를 들어보고, 그다음 노트 클립으로 세부 판단을 적어주세요.</p>",
            "  <table>",
            "    <thead>",
            "      <tr>",
            "        <th>노트</th>",

...
def test_notes_with_sync_offset_preserves_negative_layer_shift() -> None:
    ...
    shifted = notes_with_sync_offset([note], sync_offset_seconds=-0.25, bpm=120)

    assert shifted[0].onset_seconds == -0.25
    assert shifted[0].beat == 0.5


def test_notes_with_sync_offset_sets_voice_index_without_rewriting_existing_voice() -> None:
    ...
    shifted = notes_with_sync_offset([missing_voice, existing_voice], sync_offset_seconds=0.5, bpm=120, voice_index=2)

...
PdfPreflightKind = Literal["score_like", "image_score_possible", "not_score", "unknown"]
...
def inspect_pdf_for_score_content(path: Path, *, max_pages: int = 3) -> PdfPreflightResult:
    ...
    The check is deliberately conservative. Image-heavy PDFs may be scanned
    scores, so they remain eligible for OMR. Text-only PDFs without staff or
    note evidence are treated as lyrics/general documents.
    """
    ...
    with document:
        ...
        if page_count <= 0:
            return PdfPreflightResult(kind="not_score", reason="empty_pdf")
        ...
        if staff_row_count > 0 or note_glyph_count > 0:
...
def _has_large_image(page: Any) -> bool:

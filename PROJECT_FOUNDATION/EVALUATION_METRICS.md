...
Candidate UI는 role, compact note-flow text preview, compact musical facts,
concrete review warnings를 보여준다. confidence percentages, raw score
metrics, duplicate preview widgets는 주요 판단 근거가 아니다.
...
### Audio Export Regression

Audio export is evaluated as an output rendering of the active timeline, not as
a new source of product truth.

| Metric | Meaning | Target |
| --- | --- | ---: |
| Active timeline only | Export excludes archives, candidates, and reports unless restored/approved | 100% |
| Format correctness | WAV starts with a valid RIFF/WAVE stream; MP3 is encoded from the completed WAV mix | regression pass |
| Source selection | Original audio and guide sound are used only when available for the selected track | regression pass |
| Sync preservation | Negative sync shifts the whole output to 0 without changing relative track timing | fixture pass |
